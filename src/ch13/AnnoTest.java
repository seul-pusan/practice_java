...
@Retention(RetentionPolicy.RUNTIME) // 어노테이션 적용 범위를 실행 시에도 유지될 수 있도록 설정
@interface Check {
	String name();
...
public class AnnoTest {
	...
	public static void test() {
		AnnoTest obj = new AnnoTest(); // obj는 AnnoTest 객체 참조.
		try {
			Class<?> c = obj.getClass(); // obj.getClass(): AnnoTest 객체에 대한 정보를 가지는 Class 객체를 생성해 반환
			Method m = c.getMethod("test"); // c.getMethod(): 클래스 정보 중 인자로 전달한 이름의 메서드를 Method 객체로 생성하여 반환
											// Method객체는 메서드에 대한 정보 처리를 지원

			Check check = m.getAnnotation(Check.class); // Check.class는 Check 객체에 대한 정보를 가진 Class 객체가 생성되며 getAnnotation
														// 메서도에 인자로 전달
