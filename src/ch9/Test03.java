...
public class Test03 {
	public static void main(String[] args) {
		...
		String s = "have a nice day!"; // 문자열의 위치에 따라 인덱스 지정됨. 0부터 시작

		int length = s.length(); // s변수가 참조하는 문자열의 길이 반환
		System.out.println("total number of character : " + length);

		char c;
		for (int i = 0; i < s.length(); i++) { // i가 0부터 length 값보다 작을 때까지 1씩 증가. i는 문자열 s의 인덱스로 사용
			c = s.charAt(i); // 인자로 전달받은 인덱스에 해당하는 문자 추출. c변수의 타입은 char
			System.out.print(c);
		}
	}
